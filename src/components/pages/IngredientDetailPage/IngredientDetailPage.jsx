...
export const IngredientDetail = () => {
  ...
  const { id } = useParams();
  const [ingredient, setIngredient] = useState(null); //thông tin chi tiết về ingredient
  ...
  useEffect(() => {
    document.title = "Ingredients Detail";
  });
  ...
  if (!ingredient) {
    return <div>Ingredient data is not available</div>;
  }
  ...
  const handleAddComment = async () => {
    ...
    try {
      ...
      setComments([
        ...
        ...comments,
      ]); //sao chép toàn bộ comment trước đó bao gồm cả comment mới được thêm vào danh sách
